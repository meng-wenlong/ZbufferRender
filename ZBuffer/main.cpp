...
int main(int argc, const char * argv[]) {
    ...
    mesh.bounding_sphere_c.x = (mesh.bounding_box[0].x + mesh.bounding_box[1].x) / 2;
    mesh.bounding_sphere_c.y = (mesh.bounding_box[0].y + mesh.bounding_box[1].y) / 2;
    mesh.bounding_sphere_c.z = (mesh.bounding_box[0].z + mesh.bounding_box[1].z) / 2;
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        for (int i=0; i<mesh.verts.size(); i++) {
            mesh.verts[i].x -= mesh.bounding_sphere_c.x;
            double xt = mesh.verts[i].x * cos(theta) + mesh.verts[i].z * sin(theta);
            double zt = mesh.verts[i].z * cos(theta) - mesh.verts[i].x * sin(theta);
            mesh.verts[i].x = xt + mesh.bounding_sphere_c.x;
            mesh.verts[i].z = zt;
        }
        theta += 0.01;
        
        //构建分类多边形表和分类边表，初始化活化多边形表和活化边表
